...
function starOptionIcon(q: Question | undefined | null, optText: string): ReactNode {
  const t = (q?.type || "").toLowerCase();
...
function smileOptionIcon(q: Question | undefined | null, optText: string): ReactNode {
  const t = (q?.type || "").toLowerCase();
  ...
  if (t === "smile 4") {
    ...
    if (label === "bom")
      return <Smile className="w-3.5 h-3.5 text-[#84cc15]" />;
    if (label === "regular")
      return <Meh className="w-3.5 h-3.5 text-[#e9b306]" />;
    if (label === "ruim")
      return <Frown className="w-3.5 h-3.5 text-[#ef4444]" />;
  }
  if (t === "smile 5") {
    if (label === "muito satisfeito")
      return <Laugh className="w-3.5 h-3.5 text-[#22c55d]" />;
    if (label === "satisfeito")
      return <Smile className="w-3.5 h-3.5 text-[#84cc15]" />;
    if (label === "regular")
      return <Meh className="w-3.5 h-3.5 text-[#e9b306]" />;
    if (label === "insatisfeito")
      return <Frown className="w-3.5 h-3.5 text-[#f97316]" />;
    if (label === "muito insatisfeito")
      return <Angry className="w-3.5 h-3.5 text-[#ef4444]" />;
  }
  return null;
}
...
const NPS_GROUPS = [
  {
    ...
    min: 0,
    max: 6,
    icon: <Frown className="w-3.5 h-3.5 text-[#ef4444]" />,
    color: "#ef4444",
  },
  {
    label: "7-8",
    min: 7,
    max: 8,
    icon: <Meh className="w-3.5 h-3.5 text-[#e9b306]" />,
    color: "#e9b306",
  },
  {
    label: "9-10",
    min: 9,
    max: 10,
    icon: <Laugh className="w-3.5 h-3.5 text-[#22c55d]" />,
    color: "#22c55d",
  },
] as const;
  ...
  return (
    <div className="rounded-xl bg-zinc-900 border border-emerald-500/40 text-white px-4 py-3 shadow-lg w-52 text-center">
      ...
      <div className="text-[9px] text-zinc-400 mt-1 truncate">
        {data.endMessage || "Obrigado pela sua resposta!"}
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <Handle
        ...
        position={Position.Left}
        id="in"
        style={{ background: "#0b82ff", width: 10, height: 10 }}
      />
      <div
        className={`text-[8px] font-black uppercase tracking-widest mb-1 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}
      >
        {t || "Pergunta"}
      </div>
      <div
        className={`text-xs font-semibold leading-tight min-h-[16px] ${isDarkMode ? "text-zinc-100" : "text-zinc-800"}`}
      >
        ...
        {isNps(q) ? (
          NPS_GROUPS.map((g) => {
            ...
            return (
              <div
                key={g.label}
                className={`relative flex items-center justify-between gap-2 rounded px-2 py-1 ${isDarkMode ? "bg-zinc-800/80" : "bg-zinc-50"}`}
              >
                <span className="flex items-center gap-1.5 min-w-0">
                  {g.icon}
                  <span
                    className={`text-[10px] font-bold truncate ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
                  >
                    {g.label}
                  </span>
                </span>
                <span
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${mapped ? "bg-emerald-500" : "bg-red-500"}`}
                  title={mapped ? "Conectado" : "Sem destino"}
                />
                <Handle
                  type="source"
                  position={Position.Right}
                  id={g.label}
                  style={{
                    background: mapped ? "#22c55e" : "#ef4444",
                    width: 8,
                    height: 8,
                  }}
                />
          ...
          <>
            {showOptionRows && options.length === 0 && (
              <div className="text-[9px] text-amber-600 font-bold">
                Nenhuma opção — adicione no painel
              </div>
            )}
            {showOptionRows &&
              options.map((opt) => {
                  ...
                  return (
                    ...
                    >
                      <span className="flex items-center gap-1.5 min-w-0">
                        ...
                        {!locked && opt.color && (
                          <span
                            className={`w-2.5 h-2.5 rounded-full shrink-0 border ${isDarkMode ? "border-white/10" : "border-black/10"}`}
                            style={{ background: opt.color }}
                          />
                        )}
                        <span
                          className={`text-[10px] truncate ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
                        >
                          {opt.text || "(vazio)"}
                        </span>
                      </span>
                ...
                return (
                  <div
                    key={opt.id}
                    className={`flex items-center gap-1.5 rounded px-2 py-1 ${isDarkMode ? "bg-zinc-800/80" : "bg-zinc-50"}`}
                  >
                    <span
                      className={`w-2 h-2 rounded-full shrink-0 border ${isDarkMode ? "border-white/10" : "border-black/10"}`}
                      style={{ background: opt.color || "#3b82f6" }}
                    />
                    <span
                      className={`text-[10px] truncate ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
                    >
            ...
            {!perOption && (
              <div
                className={`relative flex items-center justify-between rounded px-2 py-1 ${isDarkMode ? "bg-zinc-800/80" : "bg-zinc-50"}`}
              >
                <span
                  className={`text-[10px] ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}
                >
                  {showOptionRows ? "Continua →" : "Resposta livre"}
                </span>
                <Handle
                  type="source"
                  position={Position.Right}
                  id="next"
                  style={{
                    background: data.defaultNext ? "#22c55e" : "#ef4444",
                    width: 8,
                    height: 8,
                  }}
                />
...
function EditorInner() {
  ...
  const layout = (questionNodes: Question[], startId: string) => {
    ...
    for (const n of questionNodes) {
      ...
      if (n.branch.rules)
        for (const t of Object.values(n.branch.rules))
          if (t !== END) targets.push(t);
      if (n.branch.defaultNext && n.branch.defaultNext !== END)
        targets.push(n.branch.defaultNext);
    ...
    for (let l = 0; l <= maxLayer + 1; l++) {
      layered.push(
        ...questionNodes.filter((n) => layers.get(n.id) === l).map((n) => n.id),
      );
    }
    for (const n of questionNodes)
      if (!layers.has(n.id) && !layered.includes(n.id)) layered.push(n.id);
  ...
  const buildNodesFromGraph = (
    ...
    const qNodes: FlowNodeType[] = normalized.map((q) => ({
      id: q.id,
      type: "question",
      position: savedLayout?.[q.id] ||
        positions.get(q.id) || { x: 400, y: 200 },
    ...
    return [
      {
        id: START_ID,
        type: "start",
        position: savedLayout?.[START_ID] || startPos,
        data: { kind: "start" },
      },
      ...qNodes,
      {
        id: END_ID,
        type: "end",
        position: savedLayout?.[END_ID] || {
          x: maxX,
          y: 60 + (positions.get(startId)?.y ?? 0),
        },
        data: { kind: "end", endMessage },
      },
    ] as FlowNodeType[];
  };
  ...
  const buildEdgesFromGraph = (graph: ReturnType<typeof buildFlowGraph>) => {
    ...
    const startEdge =
      graph.startId && graph.questionNodes.some((n) => n.id === graph.startId)
        ? [
            {
              id: edgeId(START_ID, "out", graph.startId),
              source: START_ID,
              sourceHandle: "out",
              target: graph.startId,
              targetHandle: "in",
            },
          ]
        : [];
    const mapTarget = (targetId: string) =>
      targetId === END ? END_ID : targetId;
    const qEdges: Edge[] = [];
  ...
  const cleanInvalidEdges = (edges: Edge[], nodes: FlowNodeType[]): Edge[] => {
    ...
    return edges.filter((e) =>
      validHandles.has(`${e.source}::${e.sourceHandle || ""}`),
    );
  };
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const loadedNodes = buildNodesFromGraph(
          graph.questionNodes,
          graph.startId,
          graph.endMessage,
          data.flow_layout?.nodes,
        );
  ...
  useEffect(() => {
    setNodes((ns) => {
      ...
      const next = ns.map((n) => {
        ...
        const sameRules =
          JSON.stringify(n.data.rules || {}) === JSON.stringify(rules);
  ...
  const addQuestion = () => {
    ...
    setNodes((ns) => [
      ...ns,
      {
        id: q.id,
        type: "question",
        position: { x: 380 + count * 24, y: 120 + count * 60 },
        data: { kind: "question", question: q },
      },
    ]);
    setSelectedId(q.id);
  ...
  const changeType = (nodeId: string, type: string) => {
    ...
    if (preset && prev.type !== type) {
      ...
      if (preset.options) {
        q.options = preset.options.map((text, i) => ({
          id: "o" + (i + 1),
          text,
        }));
      }
    ...
    setNodes((ns) =>
      ns.map((n) =>
        n.id === nodeId ? { ...n, data: { ...n.data, question: q } } : n,
      ),
    );
    ...
    setEdges((eds) =>
      eds.filter((e) => {
        ...
        return (
          !!newPerOption && e.sourceHandle && newOptIds.has(e.sourceHandle)
        );
      }),
    );
  };
  ...
  const questionTargets = useMemo(
    () =>
      nodes
        ...
        .map((n) => ({
          id: n.id,
          label: (n.data.question?.text || "").trim() || "(sem texto)",
        })),
    [nodes],
  );
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span
            className={`text-[10px] font-bold uppercase tracking-widest ${dark ? "text-zinc-500" : "text-zinc-400"}`}
          >
            {label}
          </span>
          {image && (
        ...
        {image ? (
          <div
            className={`relative w-full h-32 rounded-lg border overflow-hidden ${dark ? "border-white/10 bg-black" : "border-zinc-200 bg-slate-50"}`}
          >
            <img
              src={image}
              alt={label}
          ...
          <label
            className={`flex items-center justify-center gap-3 w-full h-24 rounded-lg border-2 border-dashed cursor-pointer transition-colors ${dark ? "border-zinc-700 bg-black hover:border-blue-500" : "border-zinc-200 bg-slate-50 hover:border-blue-400"}`}
          >
            <Upload
              size={16}
            ...
            />
            <span
              className={`text-[10px] font-bold uppercase tracking-widest ${dark ? "text-zinc-500" : "text-slate-500"}`}
            >
              Anexar imagem
            </span>
            <input
        ...
        <p
          className={`text-[9px] font-medium ${dark ? "text-zinc-700" : "text-zinc-400"}`}
        >
  ...
  const buildPayload = () => {
    ...
    return serializeFlow({
      ...
      questionNodes,
      edges: flowEdges,
      endMessage: thankYouMessage.trim(),
    });
  };
  ...
  const handleSave = async () => {
    if (!user) return;
    if (!title.trim())
      return toast.error("O título da campanha é obrigatório.");

    const questionCount = nodes.filter(
      (n) => n.data.kind === "question",
    ).length;
    if (questionCount === 0)
      return toast.warning("Adicione pelo menos uma pergunta.");

    const hasStartEdge = edges.some((e) => e.source === START_ID);
    if (!hasStartEdge)
      return toast.warning("Conecte o Node Início à primeira pergunta.");
    ...
    try {
      ...
      toast.success(
        isEdit ? "Campanha atualizada!" : "Campanha criada com sucesso!",
      );
    ...
    <>
      ...
      <div className="flex h-[calc(100vh-56px)]">
        ...
        <div className="flex-1 relative">
          ...
          <div
            className={`absolute top-3 right-3 z-[9999] pointer-events-none select-none flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${isDarkMode ? "bg-zinc-800/80 text-amber-400 border-white/10" : "bg-white/80 text-amber-600 border-zinc-200"}`}
          >
        ...
        {/* Panel */}
        <div
          className={`w-[340px] border-l overflow-y-auto p-4 space-y-4 ${isDarkMode ? "border-white/10 bg-zinc-900" : "border-zinc-200 bg-white"}`}
        >
          {!selectedNode ? (
            <div
            ...
            >
              <p
                className={`font-bold mb-2 ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
              >
                Fluxo da Campanha
              </p>
              <p>
                Clique em um node para editar. Conecte cada opção de resposta a
                uma próxima pergunta ou à tela final.
              </p>
              <p
                className={`mt-2 text-xs ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}
              >
                • Node verde = Início
                <br />• Node escuro = Finalizar
                <br />• Conecte arrastando da bolinha colorida da opção
              </p>

              <div
                className={`mt-5 mb-3 border-t ${isDarkMode ? "border-white/10" : "border-zinc-100"}`}
              />
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <p
                    className={`font-bold ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
                  >
                    Imagens da Campanha
                  </p>
                </div>
                ...
                >
                  Imagens opcionais exibidas no início e no final da pesquisa.
                </p>
                {renderImageField(
                  "start",
                  "Imagem inicial",
            ...
            <div className="space-y-3">
              <p
                className={`font-bold text-sm ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
              >
                Node Início
              </p>
              <label
                className={`block text-xs font-semibold ${isDarkMode ? "text-zinc-400" : "text-zinc-500"}`}
              >
                Primeira pergunta
              </label>
              <select
...
function QuestionPanel({
  ...
  const showColor =
    !locked && ["Escolha Única", "Múltipla Escolha"].includes(question.type);
  const { theme } = useTheme();
  const isDarkMode = theme === "dark";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4">
        <label
          className={`flex items-center gap-1.5 text-xs ${isDarkMode ? "text-zinc-400" : "text-zinc-600"}`}
        >
          <input
            type="checkbox"
        ...
        >
          <input
            ...
            onChange={(e) =>
              onPatch(nodeId, { allowComment: e.target.checked })
            }
          />
          Permitir comentário
        </label>
        ...
        <div className="space-y-3">
          ...
          {NPS_GROUPS.map((g) => {
            ...
            return (
              <div
                key={g.label}
                className={`border rounded-md p-2 flex items-center gap-2 ${isDarkMode ? "border-white/10" : "border-zinc-100"}`}
              >
                <span className="flex items-center gap-1.5 min-w-0">
                  {g.icon}
                  <span
                    className={`text-sm font-bold truncate min-w-0 ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
                  >
                    {g.label}
                  </span>
                </span>
        ...
        <div className="space-y-3">
          ...
          {question.options?.map((opt, i) => {
            ...
            return (
              <div
                key={opt.id}
                className={`border rounded-md p-2 space-y-2 ${isDarkMode ? "border-white/10" : "border-zinc-100"}`}
              >
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-zinc-400">{i + 1}.</span>
                  {locked ? (
                    <span
                      className={`flex-1 text-sm truncate min-w-0 ${isDarkMode ? "text-zinc-200" : "text-zinc-700"}`}
                    >
                      {opt.text || "(vazio)"}
                    </span>
                  ) : (
                    <input
                      value={opt.text}
                      onChange={(e) =>
                        onRenameOption(nodeId, opt.id, e.target.value, opt.text)
                      }
                  ...
                  {showColor && (
                    <span
                      className={`relative w-6 h-6 rounded-md overflow-hidden border shrink-0 ${isDarkMode ? "border-white/10" : "border-zinc-200"}`}
                      title="Cor da opção"
                    >
                      <input
                        type="color"
                        ...
                        onChange={(e) =>
                          onChangeOptionColor(nodeId, opt.id, e.target.value)
                        }
          ...
          {!perOption && (
            <div>
              <label className={labelCls}>
                Próxima pergunta (após a resposta)
              </label>
              <select
                value={
                  defaultNext && defaultNext !== END
                    ? defaultNext
                    : defaultNext === END
                      ? END_ID
                      : ""
      ...
      <div
        className={`pt-1 text-[10px] ${isDarkMode ? "text-zinc-500" : "text-zinc-400"}`}
      >
