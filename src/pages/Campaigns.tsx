          ...
          <div className="mt-6 space-y-4">
            {loading ? (
              ...
              const columns = qType === 'SMILE 5'
                ...
                : qType === 'NPS'
                ? [
                    { key: 'perception_excelente', label: '9-10', color: '#22c55d', icon: Smile },
                    { key: 'perception_bom',       label: '7-8',  color: '#e9b306', icon: Meh },
                    { key: 'perception_ruim',      label: '0-6',  color: '#ef4444', icon: Frown },
                  ]
                : perceptionData;
              ...
              return (
                ...
                <div className={`flex items-center gap-2 md:gap-4 border-l pl-6 w-full lg:w-auto ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                  {columns.map((col, idx) => (
                    <div key={col.key + idx} className="flex flex-col items-center gap-1 min-w-[50px]">
                      <col.icon 
                        size={24} 
                        strokeWidth={2} 
                        ...
                        fill={col.color}
                      />
                      <span className={`text-[9px] font-bold uppercase tracking-tighter text-center ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>{col.label}</span>
                    </div>
                  ))}
                </div>
