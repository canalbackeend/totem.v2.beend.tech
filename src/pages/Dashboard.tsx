...
export default function Dashboard() {
  ...
  const parseCampaignNames = (campaigns: string | null | undefined): string[] => {
    if (!campaigns) return [];
    try {
      const parsed = JSON.parse(campaigns);
      return Array.isArray(parsed) ? parsed : [];
    } catch {
      return campaigns.split(',').map(c => c.trim()).filter(Boolean);
    }
  };

  const filteredTerminals = useMemo(() => {
    if (!selectedCampaignId || !campaigns.length) return terminalsList;
    const selectedCampaign = campaigns.find(c => c.id === selectedCampaignId);
    if (!selectedCampaign) return terminalsList;
    return terminalsList.filter((term: any) => {
      const names = parseCampaignNames(term.campaigns);
      return names.includes(selectedCampaign.name);
    });
  }, [selectedCampaignId, campaigns, terminalsList]);
  ...
  useEffect(() => {
    ...
    const fetchEvolution = async () => {
      ...
      try {
        let evolutionEndpoint = `/campaigns/${selectedCampaignId}/evolution?days=7`;
        if (startDate) evolutionEndpoint += `&startDate=${startDate}`;
        if (endDate) evolutionEndpoint += `&endDate=${endDate}`;
        if (selectedTerminalId !== 'all') evolutionEndpoint += `&terminal_id=${selectedTerminalId}`;
        const data = await api.get(evolutionEndpoint);
            ...
            <div className="p-6 flex flex-wrap lg:grid lg:grid-cols-5 items-end gap-4">
              ...
              <div className="flex flex-col space-y-1.5 min-w-[160px] flex-1 lg:flex-none">
                ...
                >
                  ...
                  {filteredTerminals.length > 0 ? filteredTerminals.map((term: any) => (
